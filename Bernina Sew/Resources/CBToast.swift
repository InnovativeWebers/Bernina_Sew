...
class CBToast: NSObject {
    ...
    //显示消息
    @objc class func showToast(message : NSString?, aLocationStr : NSString?, aShowTime : TimeInterval) {
        if Thread.current.isMainThread {
            toastLabel = self.currentToastLabel()
            toastLabel?.removeFromSuperview()
            
            var superWindow: UIWindow?
            if UIApplication.shared.windows.last?.isOpaque == true && UIApplication.shared.windows.last?.isHidden == false {
                superWindow = UIApplication.shared.windows.last
            }else{
                let windows = UIApplication.shared.windows
                for window in windows.reversed() {
                    if window.bounds==UIScreen.main.bounds && window.isHidden==false && window.isOpaque==true {
                        superWindow = window
                        break
                    }
                }
            }
            superWindow?.addSubview(toastLabel!)
...
extension CBToast {
    class func main_width() ->(CGFloat) {
        let windows = UIApplication.shared.windows
        for window in windows.reversed() {
            if window.bounds==UIScreen.main.bounds && window.isHidden==false && window.isOpaque==true && (window.alpha as AnyObject !== (CGFloat)(0) as AnyObject) {
    ...
    class func main_height() ->(CGFloat) {
        let windows = UIApplication.shared.windows
        for window in windows.reversed() {
            if window.bounds==UIScreen.main.bounds && window.isHidden==false && window.isOpaque==true && (window.alpha as AnyObject !== (CGFloat)(0) as AnyObject) {
